...
class Predictor(nn.Module):
    def __init__(self,
        ...
        self.type_embeddings = torch.nn.Embedding(types, type_dim)
        self.regression_embedding = torch.nn.Embedding(1, transformer_width)
        ...
        # 4. (B x nobj(+1) x transformer_input_d) -> (B x nobj x out_dim=1)
        final_lin = [torch.nn.Linear(transformer_width, final_lin_unit[0])]
        for i in range(1, len(final_lin_unit)):
            final_lin.append(torch.nn.Linear(final_lin_unit[i-1], final_lin_unit[i]))
        self.final_lin = torch.nn.ModuleList(final_lin)
    ...
    def forward(self, x, padding_mask, inpaint_mask, padding_inpant, condition = None, un_condition = None, word = None, cond_scale_guidance = 1.0):
        """ x           : [batch_size, maxnumobj, pos+ang+siz+cla]
            padding_mask: [batch_size, maxnumobj], for nn.TransformerEncoder (False: not masked, True=masked, not attended to)
            inpaint_mask: [batch_size, maxnummask, pos+ang+siz+1]
            padding_inpant: [batch_size, maxnummask], for nn.TransformerEncoder (False: not masked, True=masked, not attended to)
        ...
        reg_index = torch.zeros((initial_feat.shape[0], ), dtype=torch.long).to(x.device)
        reg_embed = self.regression_embedding(reg_index).unsqueeze(1) # [B, 1, transformer_input_d]
        padding_mask = torch.cat([padding_mask, torch.zeros_like(padding_mask[:, [0]])], dim=1)
        initial_feat = torch.cat([initial_feat, reg_embed], dim=1) # [B, 1+numoftoken+initial_feat, 512(-type)]
        ...
        for i in range(len(self.final_lin)-1):
            trans_out = self.activation(self.final_lin[i](trans_out))
        out = self.final_lin[-1](trans_out) # (B x 1 x out_dim) 
        
        return torch.sigmoid(out.squeeze(1)) # classifier (classificaiton token is the last entry along dim1)